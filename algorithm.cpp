...
string aStar_ExpandedList(string const initialState, string const goalState, int &numOfStateExpansions, int& maxQLength,
   ...
   Queue searchQueue, tempSearchQueue;
   string path;
   clock_t startTime;
   cout << "------------------------------" << endl;
   cout << "<<       A* Algorithm       >>" << endl;
   cout << "------------------------------" << endl;
   Puzzle *tempSearchState, *searchState, *foundGoalState;
   int maxDepth, count, i = 0, misplacedTilesValue = 0;
   maxDepth = count = 1;
   bool goalFound = false, levelState = false;
   startTime = clock();
   while (!goalFound) {
       //Add start node to Queue to restart searching at deeper depth.
       Puzzle *startState = new Puzzle(initialState, goalState);
       Puzzle *tempSearchState = new Puzzle(initialState, goalState);
       searchState = startState;
       ...
       //do {
           ...
           if (searchState->goalMatch()) {
             ...
             if (searchState->canMoveLeft(maxDepth)) {
              ...
              //Puzzle *tempSearchState = new Puzzle(initialState, goalState);
              tempSearchQueue = searchQueue;
              cout<<"This is the searchQueue"<<endl;
              searchQueue.PrintAllElements();
              cout<<"This is the tempsearchQueue"<<endl;
              searchQueue.PrintAllElements();

              //searchQueue.Leave();
             //Problem with the state change/hold/this fucntion
              int numOfMisplacedTiles = searchState->h(misplacedTiles);
              for (int queueItemsCount = searchQueue.getCount(),  i = 0; i < queueItemsCount; i++) {
                if(numOfMisplacedTiles == searchQueue.Front()->h(misplacedTiles)){
                  cout<<"number of misplacedTiles: "<<numOfMisplacedTiles<<"      searchQueue.Front: " <<searchQueue.Front()->h(misplacedTiles)<<endl;
                  //searchQueue.PrintAllElements();
                  //cout<<searchState->h(manhattanDistance)<<"<---vs--->"<< searchQueue.Front()->h(manhattanDistance)<<endl;
                  min(searchQueue.Front()->h(manhattanDistance), tempSearchQueue.Front()->h(manhattanDistance));
                  searchQueue.Leave();
                }

                if(numOfMisplacedTiles > searchQueue.Front()->h(misplacedTiles)){
                  //cout<<"numOfMisplacedTiles is smaller and now changing from("<<numOfMisplacedTiles<<") to ("<<searchQueue.Front()->h(misplacedTiles)<<")."<<endl;
                  numOfMisplacedTiles = searchQueue.Front()->h(misplacedTiles);
                  tempSearchQueue = searchQueue;
                  searchQueue.Leave();
                }
                else{
              ...
              //searchQueue.PrintAllElements();
               cout<<"end of while do loop. Add prints."<<endl;
...
if (goalFound) {
    ...
    cout << "-----No Goal Found!-----" << endl;
    path = "";  //this is just a dummy path for testing the function
}
